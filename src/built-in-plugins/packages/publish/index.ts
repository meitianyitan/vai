...
export default async (packageName: string, semverStr: semver.ReleaseType) => {
  ...
  const projectType = _.get(packageInfo.packageJson, 'vai.type', null);
  if (projectType && projectType !== 'component') {
    // Is vai and only support component
    logError(`${packageName} is a vai ${projectType}, only support publish component!`);
  }
