...
import { VAI_PACKAGE_NAME } from './constants';
...
export class Entry {
  ...
  protected getAppHeader() {
    return pipe.get(
      ...
      import { history as customHistory } from "${VAI_PACKAGE_NAME}/client"
  ...
  protected getEnvironmentBody() {
    return pipe.get(
      ...
      if ((window as any)[tag]) {
        vaiStore = (window as any)[tag];
      } else {
        (window as any)[tag] = vaiStore;
      }
    `
    );
  }
