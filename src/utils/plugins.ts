...
const getBuiltInPlugins = () => {
  const plugins = [
    ['vai-plugin-command-dev', '../built-in-plugins/command-dev/index.js'],
    ['vai-plugin-command-docs', '../built-in-plugins/command-docs/index.js'],
    ['vai-plugin-command-build', '../built-in-plugins/command-build/index.js'],
    ['vai-plugin-command-bundle', '../built-in-plugins/command-bundle/index.js'],
    ['vai-plugin-command-init', '../built-in-plugins/command-init/index.js'],
    ['vai-plugin-command-preview', '../built-in-plugins/command-preview/index.js'],
    ['vai-plugin-command-test', '../built-in-plugins/command-test/index.js'],
    ['vai-plugin-project-analyse-config', '../built-in-plugins/project-analyse-config/index.js'],
    ['vai-plugin-project-analyse-layouts', '../built-in-plugins/project-analyse-layouts/index.js'],
    ['vai-plugin-project-analyse-markdown-layouts', '../built-in-plugins/project-analyse-markdown-layouts/index.js'],
    ['vai-plugin-project-analyse-markdown-pages', '../built-in-plugins/project-analyse-markdown-pages/index.js'],
    ['vai-plugin-project-analyse-not-found', '../built-in-plugins/project-analyse-not-found/index.js'],
    ['vai-plugin-project-analyse-pages', '../built-in-plugins/project-analyse-pages/index.js'],
    ['vai-plugin-white-files', '../built-in-plugins/white-files/index.js'],
    ['vai-plugin-ensure-project-files', '../built-in-plugins/ensure-project-files/index.js'],
    ['vai-plugin-service-worker', '../built-in-plugins/service-worker/index.js'],
    ['vai-plugin-mocks', '../built-in-plugins/mocks/index.js'],
    ['vai-plugin-client-ssr', '../built-in-plugins/client-ssr/index.js'],
    ['vai-plugin-command-analyse', '../built-in-plugins/command-analyse/index.js'],
    ['vai-plugin-packages', '../built-in-plugins/packages/index.js']
  ];
...
export const loadPlugins = async () => {
  await spinner('load plugins', async () => {
    ...
    getVaiPlugins(path.join(globalState.projectRootPath, 'package.json'), builtInPlugins);

    if (loadedPlugins.size > 1) {
      for (const eachPlugin of getPluginsByOrder()) {
        await Promise.resolve(eachPlugin.instance(vai));
      }
    }
  });
};

function getVaiPlugins(packageJsonPath: string, extendPlugins: any = {}) {
  ...
  Object.keys(allDependencies)
    .filter(subPackageName => subPackageName.startsWith('vai-plugin') || subPackageName.startsWith('@ali/vai-plugin'))
    .map(subPackageName => {
      ...
      loadedPlugins.add({
        ...
        config: subPackageJson && subPackageJson.vai
      });

      if (subPackageAbsolutePath) {
        getVaiPlugins(subPackageAbsolutePath);
      }
    });
}
